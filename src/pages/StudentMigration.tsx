...
import type { MigrationRequest } from '../types/supabase';
...
export default function StudentMigration() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!reason || !selectedFile || !user) return;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-2xl font-bold">Migration Certificate Requests</h1>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Reason for Migration</label>
            <textarea
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
              rows={3}
              placeholder="Please explain your reason for requesting a migration certificate..."
              required
            />
          </div>
